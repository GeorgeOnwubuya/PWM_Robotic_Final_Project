...
(
   ...
   output logic [7:0] AN,
   // pwm
   output logic [N_PWM-1:0] pwm_out         
);
   ...
   generate
      genvar i;
      for (i=4; i<5; i=i+1) begin:  unused_slot_gen_4to5
         assign rd_data_array[i] = 32'hffffffff;
      end
   endgenerate
   
   //Slot 6: PWM
   PWM_RoboticArm #(.W(N_PWM)) pwm_slot6
   (.clk(clk),
    .reset(reset),
    .cs(cs_array[`S6_PWM]),
    .read(mem_wr_array[`S6_PWM]),

...
public:
   enum {
      SW_DATA_REG = 0 //register for switches
   };

   enum {
	  BTN_DATA_REG = 1 //register for buttons
   };

   GpiCore(uint32_t core_base_addr);
   ~GpiCore(); // not used

   uint32_t read_sw(); //read switches (32-bit input)

   int read_sw(int bit_pos); //read switches at a specific position

   uint32_t read_btn(); //read buttons (32-bit input)

   int read_btn(int bit_pos); //read buttons at a specific position

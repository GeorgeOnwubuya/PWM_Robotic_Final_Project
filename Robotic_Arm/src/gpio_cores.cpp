...
uint32_t GpiCore::read_sw() {
   return (io_read(base_addr, SW_DATA_REG));
}

int GpiCore::read_sw(int bit_pos) {
   uint32_t rd_data = io_read(base_addr, SW_DATA_REG);
   return ((int) bit_read(rd_data, bit_pos));
}

uint32_t GpiCore::read_btn() {
	return (io_read(base_addr, BTN_DATA_REG));
}

int GpiCore::read_btn(int bit_pos) {
	uint32_t rd_data = io_read(base_addr, BTN_DATA_REG);
	return((int) bit_read(rd_data, bit_pos));
}
...
void PwmCore::set_duty(double f, int channel) {
   int duty;
   duty = (int) (f * (MAX/10.0)); //divide by 10 to set the duty cycle between 5% and 10%
